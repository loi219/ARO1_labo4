...
\paragraph Le registre à décalage 4 bits permet d'effectuer un décalage de tous les bits sur la droite ou sur la gauche. L'entrée D représente les bits qu'on veut pouvoir décaler.
L'entrée S permet de choisir l'action à effectuer: \begin{itemize}
...
\end{itemize}
\paragraph
L'entrée CLK est l'horloge. Lorsque l'horloge passe à 1 (flanc montant), la valeur des bits dans les balances sont changées, et lorsqu'elle passe à 0, la valeur des bits est conservée.
Ainsi, pout effectuer un décalage, il sera tout d'abord nécessaire de mettre l'entrée S à 1 et d'avoir un flanc montant pour que les bits des balances prennent la valeur de l'entrée D. Ensuite, on peut changer la valeur de l'entrée S dans le but d'avoir un décalage sur la droite ou sur la gauche. Dès lors, à chaque fois que l'horloge passera de 0 à 1, les bits vont tous se décaler d'un cran.
%fin de l'encadré
\end{tcolorbox}
